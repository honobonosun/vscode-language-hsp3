...
class Lexer {
   ...
   * @param char 評価されたい文字
   */
  private matchChar(regexp: RegExp, char: string | 0) {
    if (char === 0) { return false; }
    else { return regexp.test(char); }
  }
   ...
   * @param point 足すpoint
   */
  private addTextPoint(point: IPoint) {
  ...
  public nextToken(): Token {
    ...
    switch (this.ch) {
      ...
      case "\r": {
        if (this.peekChar() === "\n") {
          this.readChar();
          location = { begin: { row, column }, end: { row, column: column + 2 }, };
          result = new Token(TokenType.newline, location, "\r\n");
        } else {
          location = { begin: { row, column }, end: { row, column: column + 1 }, };
      ...
      default: {
        ...
        if (this.ch === 0) { // End Of Line
          ...
          result = new Token(TokenType.others, location, <string>this.ch);
        }
        break;
      }
    }
  ...
  private readSpace(): Token | null {
    ...
    while (this.matchChar(regexp, this.ch)) { this.readChar(); }
  ...
  private readComment(): Token | null {
    const beginComment = () => {
      if (this.ch === 0) { return null; }
      else if (this.ch === ";") { return ";"; }
      else if (this.ch === "/" && this.peekChar() === "/") { return "//"; }
      else if (this.ch === "/" && this.peekChar() === "*") { return "/*"; }
      else { return null; }
    };
    ...
    const position = this.position, { row, column } = this.textPoint;
    let raw: string;
    let cntRow = 0;
    let newColumn: number = 0;

    if (begin === ";" || begin === "//") {
      for (let i = 0; i < begin.length; i++) { this.readChar(); } // コメント文字文読み進める。
      const notCommentEnd = () => {
        if (this.ch === 0) { return false; }
        return !(/\r|\n/.test(this.ch));
      };
      while (notCommentEnd()) { this.readChar(); }
      raw = this.chars.slice(position, this.position).join("");
      newColumn = column + raw.length;
    } else if (begin === "/*") {
      this.readChar(); this.readChar(); // コメント文字文読み進める。
      const notCommentEnd = () => {
        if (this.ch === 0) { return false; }
        return !(this.prevChar() === "*" && this.ch === "/");
      };
      while (notCommentEnd()) { this.readChar(); } this.readChar();
      raw = this.chars.slice(position, this.position).join("");
  ...
  private readNumber(): Token | null {
    ...
    if (this.ch === "'") {
      this.readChar();
      while (this.matchChar(/\\'/, this.prevChar() + this.ch) || this.matchChar(/[^']/, this.ch)) { this.readChar(); }
      this.readChar();
      const raw: string = this.chars.slice(position, this.position).join(""),
        location: ILocation = {
          begin: { row, column },
    ...
    // $ 0x
    if (this.ch === "$" || (this.ch === "0" && this.matchChar(/[Xx]/, this.peekChar()))) {
      if (this.ch === "0") { this.readChar(); this.readChar(); } else { this.readChar(); }
      while (this.matchChar(/[0-9a-fA-F]/, this.ch)) { this.readChar(); }
      const raw: string = this.chars.slice(position, this.position).join(""),
        location: ILocation = {
          begin: { row, column },
          end: { row, column: column + raw.length },
    ...
    // % 0b
    if (this.ch === "%" || (this.ch === "0" && this.matchChar(/[Bb]/, this.peekChar()))) {
      if (this.ch === "0") { this.readChar(); this.readChar(); } else { this.readChar(); }
      while (this.matchChar(/[01]/, this.ch)) { this.readChar(); }
      const raw: string = this.chars.slice(position, this.position).join(""),
        location: ILocation = {
          begin: { row, column },
          end: { row, column: column + raw.length },
    ...
    if (this.matchChar(/[0-9]/, this.ch)) {
      // 出現したらフラグを立てる。
      let o: boolean = false, // -+記号
        e: boolean = false, // 指数表記
        f: boolean = false, // 最後のF
        dot: boolean = false; // 小数点
      while (this.matchChar(/[0-9EeFf\-+.]/, this.ch)) {
        if (f === true) { break; } // Fが来たのにまだループしている。
        else if (f === false && this.matchChar(/[Ff]/, this.ch)) { f = true; }
        else if (e === false && this.matchChar(/[\-+]/, this.ch)) { break; }  // 指数表記じゃないのに-と+が出現した。
        else if (e === true && o === true && this.matchChar(/[\-+]/, this.ch)) { break; }  // 指数表記で-と+が二回も出現した。
        else if (e === true && o === false && this.matchChar(/[\-+]/, this.ch)) { o = true; }
        else if (e === true && this.matchChar(/[Ee]/, this.ch)) { break; } // 二回目の指数表記が出現した。
        else if (e === false && this.matchChar(/[Ee]/, this.ch)) { e = true; }
        else if (dot === true && this.ch === ".") { break; } // 二回目の小数点が出現した。
        else if (dot === false && this.ch === ".") { dot = true; }
        this.readChar();
      }
      const raw: string = this.chars.slice(position, this.position).join(""),
        location: ILocation = {
      ...
      let type: TokenType;
      if (dot || e || f) { type = TokenType.double; } else { type = TokenType.int; }
      this.addTextPoint({ row: 0, column: raw.length });
  ...
  private readString(): Token | null {
    ...
    if (this.ch === "\"") {
      ...
      this.readChar(); this.readChar();
      while (true) {
        if (this.matchChar(/\\|¥/, this.ch)) { this.readChar(); this.readChar(); }
        if (this.matchChar(/"}/, this.prevChar() + this.ch) || isCH0()) { break; }
        this.readChar();
      }
      this.readChar(); this.readChar();
      const raw: string = this.chars.slice(position, this.position).join("");
  ...
  private isLiteral(): boolean {
    if (this.ch === 0) {
      ...
      if (emojiRegex().test(this.ch)) { return true; }  // emojiは識別子になれる。
      return ! /[\[\]<>{}()\-+/*\\¥&#$%^!?.,:;~=|"'\s0-9]/g.test(<string>this.ch);  // 記号と数字以外だから識別子になれる。
  ...
  private isLiteral2(): boolean {
    if (this.ch === 0) {
      ...
      if (emojiRegex().test(this.ch)) { return true; }  // emojiは識別子になれる。
      return ! /[\[\]<>{}()\-+/*\\¥&#$%^!?.,;:~=|"'\s]/g.test(<string>this.ch);  // 記号と数字以外だから識別子になれる。
    }
  }
  ...
  private readLiteral(): Token | null {
    if (this.isLiteral()) {
      ...
      while (this.isLiteral2()) { this.readChar(); }
