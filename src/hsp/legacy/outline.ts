...
export default class Outline implements vscode.Disposable {
  ...
  public update(cfg: vscode.WorkspaceConfiguration): void {
    this.masks = cfg.get("outline.masks") as string[];
    if (cfg.get("outline.enable") && !this.provider) {
      this.create();
    } else if (!cfg.get("outline.enable") && this.provider) {
      this.provider.dispose();
